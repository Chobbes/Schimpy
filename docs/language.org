...
**** C Translation
     ...
     #+END_SRC
*** Write
    Write is essentially a predefined function which sets the value of
    an output when given the output, and a boolean value to change the
    output's value to.

    #+BEGIN_SRC bnf
      write = "(", [whitespace], "Write", whitespace, variable, whitespace, expression, [whitespace], ")"
    #+END_SRC

    Note that the variable must be an output variable, and the
